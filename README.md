...
<title>Observatório Econômico de Mariana - MG</title>
    ...
    <script>
        function filtrarBoletins() {
            ...
            boletins.forEach(boletim => {
                ...
                if (anoSelecionado === 'todos' || anoSelecionado === anoBoletim) {
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        // Dados fictícios para Mariana - MG (como não tenho acesso aos dados reais em tempo real)
        // Valores baseados na tendência nacional com ajustes regionais
        const ctx = document.getElementById('ipcaGrupoChartCanvas').getContext('2d');
        
        const ipcaChart = new Chart(ctx, {
            type: 'bar',
            data: {
                labels: [
                ...
                datasets: [{
                    ...
                    data: [0.82, 0.45, 0.31, 0.28, 0.95, 0.38, 0.42, 0.15, 0.12],
                    backgroundColor: [
                        'rgba(255, 99, 132, 0.7)',
                        'rgba(54, 162, 235, 0.7)',
                        'rgba(255, 206, 86, 0.7)',
...
<script>
  const ctxEtaria = document.getElementById('populacaoFaixaEtariaChartCanvas').getContext('2d');
  ...
  const homens = [-600, -700, -750, -800, -850, -900, -950, -920, -870, -820, -780, -730, -600, -450, -300, -200, -150];
  const mulheres = [580, 690, 740, 790, 830, 890, 940, 910, 860, 810, 770, 720, 610, 470, 350, 250, 200];
        ...
        tooltip: {
          callbacks: {
            label: function(context) {
              ...
              return context.dataset.label + ': ' + val.toLocaleString() + ' pessoas';
            }
          }
        }
      },
